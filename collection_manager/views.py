...
#
from collection_manager.serializer import PaisSerializer, PuertoSerializer, TipoOperacionSerializer, AduanaSerializer, TipoCargaSerializer, ViaTransporteSerializer, RegimenImportacionSerializer, ModalidadVentaSerializer, RegionSerializer, UnidadMedidaSerializer, TipoMonedaSerializer, ClausulaSerializer, SectorSerializer
...
class PuertoViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        """Obtener un puerto específico."""
        puerto = self.get_object()
        serializer = self.get_serializer(puerto)
        return Response(serializer.data)
    ...
    def update(self, request, pk=None):
        ...
        puerto = self.get_object()
        serializer = self.get_serializer(puerto, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
