...
def train_lstm_all_ports_multistep(
    ...
    # Para visualización, seleccionaremos una muestra aleatoria del conjunto de test
    sample_idx = random.randint(0, X_test.shape[0] - 1)
    sample_actual = y_test_inv[sample_idx]
    sample_pred = y_pred[sample_idx]

    # Crear un rango de semanas futuras
    weeks_future = np.arange(1, n_future + 1)

    # # Plotting Predicciones vs Reales
    # plt.figure(figsize=(10, 6))
    # sns.lineplot(x=weeks_future, y=sample_actual, marker='o', label='Reales')
    # sns.lineplot(x=weeks_future, y=sample_pred, marker='x', label='Predichas')
    # plt.title(f"Predicción vs Real para una muestra del conjunto de test (PUERTOEMB {puertoemb})")
    # plt.xlabel("Semana futura")
    # plt.ylabel("PESOBRUTOTOTAL")
    # plt.legend()
    # plt.grid(True)
    # plt.tight_layout()
    # plt.savefig("predicciones_vs_reales.png")
    # plt.show()
    # logging.info("Gráfico de predicciones vs reales generado y guardado como 'predicciones_vs_reales.png'.")

    # Visualizar el historial de entrenamiento (pérdida)
    # plt.figure(figsize=(10, 6))
